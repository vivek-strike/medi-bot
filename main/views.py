...
from .models import Doctor_consultation, DoctorAdd, Hospital
...
def ViewHospital(request):
    ...
    hospitals = Hospital.objects.filter(Symptoms__contains = disease) 
    ...
    return render(request,"viewhospital.html",context)
...
from .forms import UserRegistration, ConsultationaddForm
...
def add_doctor(request):
    ...
    return render(request,'main/add_doctor.html',context)

def deletedoctor(request,pk):
    DoctorAdd.objects.get(id = pk).delete()
    messages.info(request,"Doctor delated....")
    return redirect("add_doctor")
...
def AddHospital(request):
    hospitals = Hospital.objects.all()[::-1]
    symptoms = ['Fungal infection','Allergy','GERD','Chronic cholestasis','Drug Reaction',
        'Peptic ulcer diseae','AIDS','Diabetes','Gastroenteritis','Bronchial Asthma','Hypertension',
        ' Migraine','Cervical spondylosis',
        'Paralysis (brain hemorrhage)','Jaundice','Malaria','Chicken pox','Dengue','Typhoid','hepatitis A',
        'Hepatitis B','Hepatitis C','Hepatitis D','Hepatitis E','Alcoholic hepatitis','Tuberculosis',
    ...
    if request.method == "POST":
        ...
        symptom = request.POST.getlist('symptoms[]')
        ...
        hospital.save()
        messages.info(request,"Hospital Addedd........")
        return redirect("AddHospital")

    return render(request,"main/addhospitel.html",{"symptoms":symptoms,"hospitals":hospitals})

def deletehospital(request,pk):
    Hospital.objects.get(id = pk).delete()
    messages.info(request,"Hospital Deleted......")
    return redirect("AddHospital")


#appontment taken .................................

def GetAppointment(requets,pk):
    hospital = Hospital.objects.get(id = pk)
    doctor = DoctorAdd.objects.filter(hospital = hospital)
    form = ConsultationaddForm()
    if requets.method == "POST":
        form = ConsultationaddForm(requets.POST)
        doctor = requets.POST['doctor']
        if form.is_valid():
            consultat = form.save()
            consultat.Doctor_name = DoctorAdd.objects.get(id = int(doctor))
            consultat.hospital = hospital
            consultat.user = requets.user
            consultat.save()
            messages.info(requets,"consultation booked")
            return redirect("consultation_request")
    ...
    return render(requets,"main/getappointment.html",context)

def consultation_request(request):
    
    requ = Doctor_consultation.objects.filter(user = request.user)
...
def consultation_request_admin(request):
    ...
    return render(request,'main/consultaion_request_admin.html',{'requests':requ})


def MedReminder(request):
